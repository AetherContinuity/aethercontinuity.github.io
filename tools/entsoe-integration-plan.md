...
- `/installed-capacity` (A68, vuositason kapasiteetti) — **VAHVISTETTU TOIMIVAKSI 2026-07-24**, mutta VAATI korjauksen: SE1 (tarjousalue) palautti 'No matching data' JOKAISELLE yhdistelmälle (2025/2026, B19-suodatinilla ja ilman) — juurisyy löytyi: Ruotsi raportoi tämän artiklan vain KOKO MAAN tasolla (`bzn=SE`, EIC `10YSE-1--------K`), ei yksittäisille tarjousalueille (SE1-4). Lisätty SE-koodi, testattu ja vahvistettu toimivaksi — palautti realistiset arvot (ydinvoima 6900 MW, tuulivoima 18000 MW, vesivoima 16400 MW, täsmäävät tunnettuun todellisuuteen). Sivuhavainto: resoluutio tälle artiklalle on `"P1Y"` (ei `PT<n>M`) — `resolutionToMinutes` ei tunnista tätä muotoa, mutta yhden pisteen sarjoissa tämä ei aiheuttanut ongelmaa (fallback-logiikka toimi). Jos joskus kysytään monivuotista P1Y-sarjaa, `resolutionToMinutes` pitää laajentaa tunnistamaan myös `P<n>Y`.
  ...
  **Suomi (`bzn=FI`) testattu samana päivänä** — toimi SUORAAN ilman maakoodikiertoa (Suomi on yhden vyöhykkeen maa, EIC `10YFI-1--------U` = sekä tarjousalue että maa). Palautti 12 tuotantotyyppiä (huomattavasti tarkempi erittely kuin Ruotsin 5): ydinvoima (B14) 4364 MW — täsmää tarkasti tunnettuun todellisuuteen (OL1+OL2+OL3+Loviisa 1+2 OL3:n käyttöönoton jälkeen), tuulivoima (B19) 8224 MW, vesivoima (B11) 3195 MW.

  **Aurinkovoima (B16) 46,6 MW — vahvistettu merkittäväksi raportointiaukoksi, ei koodivirhe.** Energiaviraston omien tilastojen mukaan Suomen TODELLINEN asennettu aurinkokapasiteetti oli n. 1600 MW vuoden 2025 lopussa (1247 MW jo 2024 lopussa) — ENTSO-E:n B16-luku on siis vain n. 1/34 todellisesta. Syy löytyi: valtaosa Suomen aurinkokapasiteetista (n. 1270 MW) on PIENTUOTANTOA (kotitalous-/taloyhtiökattoasennukset alle 1 MW), joka ei kulje saman kanavan kautta kuin suurten voimalaitosten raportointi Energiaviraston voimalaitosrekisteriin — vain teollisen kokoluokan laitokset (yli 1 MW, n. 326-352 MW vuoden 2025 lopussa) ovat rekisterissä, ja ENTSO-E:n oma 14.1.A-raportointi näyttää kattavan vain murto-osan siitäkin. **Tämä on sama rakenteellinen ilmiö jonka WEM oli jo aiemmin itse dokumentoinut §07:ssä** ("Rakenteellinen rajoite 2 — Fingridin oma tuotantomuotojaottelu on suuntaa-antava... perustuu Energiaviraston voimalaitosrekisteriin") — nyt löydetty ja mitattu myös ENTSO-E:n kansainvälisessä raportointiketjussa, ei vain Fingridin omassa live-datassa. Käytännön johtopäätös: ENTSO-E:n B16-lukua EI PIDÄ käyttää Suomen aurinkokapasiteetin arviona sellaisenaan missään WEM:n tulevassa laskennassa.

  **Kahden maan vertailu paljastaa tärkeän rakenteellisen eron**: sama artikla (14.1.A) raportoidaan Suomessa tarjousaluetasolla mutta Ruotsissa vain maatasolla — tämä EI ole koodivirhe vaan TSO-kohtainen raportointikäytäntö, syytä pitää mielessä kun laajennetaan muihin monivyöhykemaihin (Norja NO1-5, Tanska DK1-2) — sama SE-tyylinen maakoodikierto saattaa tarvita myös näille.
